...
#endif  // USE_ESP32_VARIANT_ESP32P4#include "mipi_camera_web_server.h"
...
esp_err_t MipiCameraWebServer::stream_handler_(httpd_req_t *req) {
  ...
  uint8_t *jpeg_data = nullptr;
  size_t jpeg_size = 0;
  bool ok = server->encode_jpeg_(rgb565, w, h, &jpeg_data, &jpeg_size, 12);
...
esp_err_t MipiCameraWebServer::snapshot_handler_(httpd_req_t *req) {
  ...
  uint8_t *jpeg_data = nullptr;
  size_t jpeg_size = 0;
  bool ok = server->encode_jpeg_(rgb565, w, h, &jpeg_data, &jpeg_size, 15);
